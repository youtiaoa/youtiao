...
{
    public class DeclformController : BaseController
    {
            ...
            if ("1".Equals(msg.ResultInfo.Status))
            {
                ...
                if (ModelState.IsValid)
                {
                    try
                    {
                        ...
                        msg.ResultInfo.Remark = string.Format("出口清单导入成功。");
                    }
                    catch (Exception ex)
                    {
        ...
        private string FormatMsg(string msg,string declformId,string logiEnteCode)
        {
            return string.Format("declform_ID={0}且LOGI_ENTE_CODE={1}的清单，{2}",declformId,logiEnteCode,msg);
        }
    }
}