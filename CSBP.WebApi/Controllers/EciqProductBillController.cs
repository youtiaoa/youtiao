...
{
    public class EciqProductBillController : BaseController
    {
        private EciqProductBillService service = new EciqProductBillService();
        private UserService _userService = new UserService();

        public void PrepareCuInsertInfo(EciqProductBillHead entity,User user)
        {
            entity.PRODUCT_FILING_INFO_ID = BillNoGenerator.NewEciqProductBill();
            ...
            foreach(var e in entity.EciqProductBillLists)
            {
                e.PRODUCT_FILING_INFO_ID = entity.PRODUCT_FILING_INFO_ID;
            }
            
        }

        // POST api/<controller>
        public IHttpActionResult PostDeclform(string token, EciqProductBillModel declform)
        {
            ...
            if ("1".Equals(msg.ResultInfo.Status))
            {
                if (ModelState.IsValid)
                {
                    try
                    {
                        ...
                        msg.ResultInfo.Remark = string.Format("国检版商品备案单导入成功。");
                    }
                    catch (Exception ex)
                    {
