...
{
    public class EciqWayBillController : BaseController
    {
        EciqWayBillService service = new EciqWayBillService();
        private UserService _userService = new UserService();

        public void PrepareCuInsertInfo(EciqWaybillHead entity, User user, string guid)
        {
            if (guid != null)
            {
                entity.GOODS_DECLAR_CHECK_ID = guid.Substring(0,32);
            }
            else
            {
                entity.GOODS_DECLAR_CHECK_ID = BillNoGenerator.NewEciqWayBill();
            }
        ...
        public IHttpActionResult PostWayBill(string token, EciqWayBillModel wayBill)
        {
            ...
            msg.ResultInfo.ID = wayBill.ORDER_NUMBER;
            if ("1".Equals(msg.ResultInfo.Status))
            {
                if (ModelState.IsValid)
                {
                    try
                    {
                        ...
                        msg.ResultInfo.Remark = string.Format("国检版运单导入成功。");
                    }
                    catch (Exception ex)
                    {
