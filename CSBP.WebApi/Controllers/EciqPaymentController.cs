...
{
    public class EciqPaymentController : BaseController
    {
        EciqPaymentBillService service = new EciqPaymentBillService();
        private UserService _userService = new UserService();
        public void PrepareCuInsertInfo(EciqPaymentHead entity,User user)
        {
            entity.PAY_ID = BillNoGenerator.NewEciqPaymentBillNo();
        ...
        public IHttpActionResult PostPayment(string token, EciqPaymentBillModel paymentBill)
        {
            ResultMsg msg = new ResultMsg();
            ResultInfo info = new ResultInfo();
            msg.ResultInfo = info;
            var user = CertValid(token, paymentBill, msg);
            msg.ResultInfo.ID = paymentBill.ORDER_NO;
            if ("1".Equals(msg.ResultInfo.Status))
            {
                if (ModelState.IsValid)
                {
                    try
                    {
                        ...
                        msg.ResultInfo.Remark = string.Format("国检版支付单导入成功。");
                    }
                    catch (Exception ex)
                    {
